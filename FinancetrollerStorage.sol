...
contract FinancetrollerV2Storage is FinancetrollerV1Storage {
    struct Market {
        ...
        // Whether or not this market receives EMCF
        bool isFinanceed;
    }
...
contract FinancetrollerV3Storage is FinancetrollerV2Storage {
    ...
    /// @notice The rate at which the flywheel distributes EMCF, per block
    ...
    /// @notice The EMCF market supply state for each market
    mapping(address => FinanceMarketState) public financeSupplyState;

    /// @notice The EMCF market borrow state for each market
    mapping(address => FinanceMarketState) public financeBorrowState;

    /// @notice The EMCF borrow index for each market for each supplier as of the last time they accrued EMCF
    mapping(address => mapping(address => uint)) public financeSupplierIndex;

    /// @notice The EMCF borrow index for each market for each borrower as of the last time they accrued EMCF
    mapping(address => mapping(address => uint)) public financeBorrowerIndex;

    /// @notice The EMCF accrued but not yet transferred to each user
...
contract FinancetrollerV5Storage is FinancetrollerV4Storage {
    /// @notice The portion of EMCF that each contributor receives per block
    mapping(address => uint) public financeContributorSpeeds;

    /// @notice Last block at which a contributor's EMCF rewards have been allocated
...
contract FinancetrollerV7Storage is FinancetrollerV6Storage {
    /// @notice Flag indicating whether the function to fix EMCF accruals has been executed (RE: proposal 62 bug)
    bool public proposal65FixExecuted;

    /// @notice Accounting storage mapping account addresses to how much EMCF they owe the protocol.
